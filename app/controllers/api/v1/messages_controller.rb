class Api::V1::MessagesController < ApplicationController
  ...
  def create
    @sender = User.find_by(username: params[:followed])
    @fan = Message.new(followed_id: @followed.id, follower_id: params[:fan][:follower_id])
    if @fan.new_record?
      @fan.save
...
def destroy
  ...
  @fan = Message.find_by(followed_id: @user_followed.id, follower_id: @user_follower.id)
  @fan.destroy
